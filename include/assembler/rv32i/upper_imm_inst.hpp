#pragma once

// assembler
#include <assembler/instruction_base.hpp>
#include <assembler/rv32i/ctx.hpp>

// C++ STL
#include <boost/asio/buffer.hpp>

namespace assembler::rv32i::u_type {
class upper_imm_inst : public instruction_base {
public:
    using rv32i_asm_struct = rv32i_context::u_type_rv32i_asm_struct_le;

    upper_imm_inst() = default;
    virtual ~upper_imm_inst() = default;

    boost::asio::mutable_buffer encode(
        tokenizer tokens,
        boost::asio::mutable_buffer rcv_buf) const override final;

    virtual void encode(tokenizer tokens,
                        rv32i_asm_struct& asm_struct) const = 0;
};
struct lui : public upper_imm_inst {
    void encode(tokenizer tokens, rv32i_asm_struct& asm_struct) const override {
        asm_struct.opcode = 0b0110111;
    }
};
struct auipc : public upper_imm_inst {
    void encode(tokenizer tokens, rv32i_asm_struct& asm_struct) const override {
        asm_struct.opcode = 0b0010111;
    }
};

}  // namespace assembler::rv32i::u_type